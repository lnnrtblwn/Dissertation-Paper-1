...
k = 20 # number of time series. already tried with 1000 (half sample)/ 1000 (iterations)/ 20 (series share): 5
p = 1 # AR specification of VAR-Process
iter =  50 # iterations per MC-simulation
...
for (series_share in seq(0, 1, by = 1 / 20)) {
  ...
  for (iteration in c(1:iter)) {
    ...
    for (i in (p + 1):(t_full + 2 * p)) {
      series_VAR[, i] <-
        A %*% series_VAR[, i - 1] + rnorm(k, 0, 0.5) # Generate series with e ~ N(0,0.5)
    }
    ...
    # convex optimization via CVXR package.
